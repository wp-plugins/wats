...
{
    global $wats_settings, $wats_default_ticket_status, $wats_version, $wats_default_ticket_priority, $wats_default_ticket_type, $wats_default_sla;
...
function wats_options_admin_menu()
{
	...
	if (isset($_POST['save']))
	{
		...
		$wats_settings['default_ticket_product'] = $_POST['group_default_wats_products'];
	...
	if ($wats_settings['ticket_visibility_read_only_capability'] == 1)
		echo ' checked';
	echo '> '.__('Grant read only access to all tickets for users with "wats_ticket_read_only" capability','WATS').'</div></td></tr><tr><td>';
	...
	foreach ($roles AS $role)
	{
		...
		if ($wats_settings['ticket_assignment_'.strtolower($role['name'])] == 1)
			echo ' checked';
	...
	foreach ($roles AS $role)
	{
		...
		if ($wats_settings['dashboard_stats_widget_'.strtolower($role['name'])] == 1)
			echo ' checked';
		echo '> '.translate_user_role($role['name']).'<br />';
	}
	echo '</td></tr><tr><td>';
	...
	wats_options_premium_only();
	echo '<br />';
	echo '<table class="widefat" cellspacing="0" id="tablesla" style="text-align:center;"><thead><tr class="thead">';
	echo '<th scope="col" class="manage-column" width="10%" style="text-align:center;">ID</th>';
