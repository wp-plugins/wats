...
$submenu_file = 'wats-edit.php';

wp_reset_vars(array('action', 'safe_mode', 'withcomments', 'posts', 'content', 'edited_post_title', 'comment_error', 'profile', 'trackback_url', 'excerpt', 'showcomments', 'commentstart', 'commentend', 'commentorder'));

function redirect_ticket($post_ID = '')
{
	global $action;
	...
	if ( !empty($_POST['mode']) && 'bookmarklet' == $_POST['mode'] ) {
		...
		$location = add_query_arg( 'message', 4, wats_get_edit_ticket_link( $post_ID, 'url' ) );
	}
...
case 'edit':
	...
	if ( empty($post->ID) ) wp_die( __("You attempted to edit a ticket that doesn't exist. Perhaps it was deleted?",'WATS') );
...
/*	if ('post' != $post->post_type) 
	...
	if ( current_user_can('edit_post', $post_ID) ) {
		if ( $last = wp_check_post_lock( $post->ID ) ) {
			...
			$message = sprintf( __('Warning: %s is currently editing this ticket','WATS'), wp_specialchars( $last_user_name ) );
	...
	$title = __('Edit Ticket','WATS');

	if ( !current_user_can('edit_post', $post_ID) )
		die ( __('You are not allowed to edit this ticket.','WATS') );
...
case 'delete':
	...
	if ( !current_user_can('delete_post', $post_id) )
		wp_die( __('You are not allowed to delete this ticket.','WATS') );
	...
	if (strpos($sendback, 'wats-ticket.php') !== false) $sendback = admin_url('admin.php?page=wats/wats-edit.php&amp;deleted=1');
